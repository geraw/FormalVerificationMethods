...
<div class="mt-2 text-center text-[18px]">
<span class="inline-block bg-red-100 border border-red-300 text-red-800 rounded px-4 py-0.5 font-bold">שגוי</span>
</div>

<div class="mt-2 text-right text-[12.5px] leading-snug">
הטענה שקולה ל: כל ריצה <KatexInline math="\mathcal{F}'" />-הוגנת היא גם <KatexInline math="\mathcal{F}" />-הוגנת (כי <KatexInline math="TS\models_{\mathcal{F}}P\Rightarrow TS\models_{\mathcal{F}'}P" /> לכל <KatexInline math="P" /> אם"ם <KatexInline math="\mathit{FairPaths}_{\mathcal{F}'}\subseteq\mathit{FairPaths}_{\mathcal{F}}" />). ניקח <KatexInline math="Act=\{x,y,x_2,w\}" />, מצבים <KatexInline math="s_0,s_1" />: ב-<KatexInline math="s_0" /> מאופשרות <KatexInline math="x" />, <KatexInline math="y" /> (לולאות) ו-<KatexInline math="x_2" /> (ל-<KatexInline math="s_1" />); ב-<KatexInline math="s_1" /> מאופשרת רק <KatexInline math="w" /> (בחזרה ל-<KatexInline math="s_0" />) — <KatexInline math="y" /> <b>כבויה</b> ב-<KatexInline math="s_1" />.
</div>
...
<TransitionSystem
  ...
  :states="[
    { id: 's0', name: 's0', x: 70, y: 55, initial: true, initialDirection: 'top' },
    { id: 's1', name: 's1', x: 200, y: 55 }
  ]"
  :transitions="[
...
<div class="mt-1 text-center text-[13px]">
<KatexInline math="B=\{x,y\},\quad C=\{y\},\quad A=B\cap C=\{y\}\ \ (A\cap B\cap C=\{y\}\neq\emptyset)" />
</div>

<div class="mt-2 bg-slate-50 border border-slate-200 rounded p-2 text-right text-[12.5px] leading-snug">
ריצה <KatexInline math="\rho=(x_2,w)^\omega" /> (לעולם לא לוקחת <KatexInline math="x,y" />, רק מחליפה מצב): <KatexInline math="y" /> מאופשרת ב-<b>כל</b> ביקור ב-<KatexInline math="s_0" /> (אינסוף פעמים) אך <b>לא ברציפות</b> (כבויה ב-<KatexInline math="s_1" />, שמבוקר לסירוגין).
</div>

<div class="mt-2 grid grid-cols-2 gap-3 text-right text-[12.5px] leading-snug">
<div class="bg-emerald-50 border border-emerald-200 rounded p-2">
<div class="font-bold text-emerald-800 mb-1"><KatexInline math="\mathcal{F}'=\langle\emptyset,\{A\},\emptyset\rangle" /> — מתקיימת</div>
חלש-<KatexInline math="A=\{y\}" />: ההיפותזה "<KatexInline math="y" /> מאופשרת <b>ברציפות</b> מנקודה מסוימת" <b>שקרית</b> (כבויה שוב ושוב ב-<KatexInline math="s_1" />) — ולכן ההוגנות מתקיימת <b>בריק</b>.
</div>
<div class="bg-red-50 border border-red-200 rounded p-2">
<div class="font-bold text-red-800 mb-1"><KatexInline math="\mathcal{F}=\langle\emptyset,\{B\},\{C\}\rangle" /> — נכשלת</div>
חזק-<KatexInline math="C=\{y\}" />: ההיפותזה "<KatexInline math="y" /> מאופשרת <b>אינסוף פעמים</b>" <b>אמיתית</b> (בכל ביקור ב-<KatexInline math="s_0" />), אך <KatexInline math="y" /> <b>לעולם לא נלקחת</b> — ההיפותזה מתקיימת וההשלכה לא.
</div>
</div>

<div class="mt-2 bg-slate-50 border border-slate-200 rounded p-2 text-right text-[12.5px] leading-snug">
<KatexInline math="\rho" /> היא <b><KatexInline math="\mathcal{F}'" />-הוגנת אך <KatexInline math="\mathcal{F}" />-לא-הוגנת</b> — בדיוק ההפך מהדרוש. לכן קיימת <KatexInline math="P" /> (למשל: לתייג <KatexInline math="L(s_0)=\{p\},L(s_1)=\emptyset" /> ולקחת <KatexInline math="P" /> כמכיל כל עקבה חוץ מ-<KatexInline math="\mathit{trace}(\rho)=(\{p\}\emptyset)^\omega" />, שהיא העקבה היחידה של הריצה היחידה — <KatexInline math="\rho" /> עצמה — עם הרצף הזה) כך ש-<KatexInline math="TS\models_{\mathcal{F}}P" /> (אין ריצה <KatexInline math="\mathcal{F}" />-הוגנת שמפרה) אך <KatexInline math="TS\not\models_{\mathcal{F}'}P" /> (כי <KatexInline math="\rho" /> עצמה <KatexInline math="\mathcal{F}'" />-הוגנת ומפרה). <b>סתירה לטענה.</b>
</div>

<div class="mt-1 text-right text-[11.5px] text-gray-600 leading-snug">
האינטואיציה: חלש דורש תגובה רק כשמאופשר <b>ברציפות</b>, ולכן "מתחמק" בקלות (בריק) כש"מהבהב". חזק דורש תגובה כבר כש<b>מאופשר אינסוף פעמים</b> (גם אם מהבהב) — ולכן דרישה חזקה יותר, שהוגנות חלשה לא יכולה להחליף.
</div>
